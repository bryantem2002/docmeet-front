  ...
  <div class="max-w-7xl mx-auto w-full px-4 sm:px-0 font-sans">
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden mt-8 transition-colors">
      ...
      <div class="block sm:hidden divide-y divide-slate-100">
        <div v-for="req in requests" :key="'mob-'+req.id" class="p-5 hover:bg-slate-50 transition-colors">
          <div class="flex justify-between items-start mb-3">
            <div>
              <h3 class="font-bold text-slate-800 text-base">{{ req.doctor }}</h3>
              <p class="text-xs font-bold text-slate-400 mt-0.5">{{ req.id }}</p>
            </div>
            <div class="text-right">
              <span v-if="req.status === 'pending'" class="px-2.5 py-1 text-[10px] font-bold rounded-full bg-amber-50 text-amber-600 border border-amber-200">
                Pendiente
              </span>
              <span v-else-if="req.status === 'approved'" class="px-2.5 py-1 text-[10px] font-bold rounded-full bg-emerald-50 text-emerald-600 border border-emerald-200">
                Aprobada
              </span>
              <span v-else class="px-2.5 py-1 text-[10px] font-bold rounded-full bg-red-50 text-red-600 border border-red-200">
                Rechazada
              </span>
            </div>
          </div>
          <div class="flex flex-col gap-1.5 mb-4 text-sm bg-slate-50 p-3 rounded-lg border border-slate-100">
            <div class="flex justify-between items-center">
              ...
              <span class="font-medium text-slate-700">{{ req.patient }}</span>
            </div>
            <div class="flex justify-between items-center">
              <span class="text-slate-500">Fecha/Hora:</span>
              <span class="font-medium text-slate-700">{{ req.date }} a las {{ req.time }}</span>
            </div>
            <div class="flex flex-col mt-2 pt-2 border-t border-slate-200">
              <span class="text-slate-500 text-xs mb-1">Motivo:</span>
          ...
          <div class="flex gap-2">
            ...
            <template v-else>
              <div class="w-full text-center text-slate-400 text-sm font-medium py-2 border border-slate-100 rounded-xl bg-slate-50">
                Gestión completada
              </div>
            </template>
          </div>
