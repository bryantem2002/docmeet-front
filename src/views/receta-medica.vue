...
<template>
  <div class="max-w-5xl mx-auto w-full">
    ...
    <!-- Encabezado -->
    <div class="mb-8">
      <h1 class="text-2xl sm:text-3xl font-extrabold text-slate-800 dark:text-white tracking-tight">Mis Recetas Médicas</h1>
      <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Consulta tus medicamentos recetados y descarga los documentos oficiales.</p>
    </div>
    ...
    <div v-if="prescriptions.length > 0" class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div 
        ...
        :key="receta.id"
        class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-3xl p-6 shadow-[0_2px_15px_rgb(0,0,0,0.03)] hover:shadow-lg transition-all duration-300 flex flex-col h-full relative overflow-hidden group"
      >
        <!-- Detalles decorativos -->
        <div class="absolute top-0 right-0 w-24 h-24 bg-[#418FC8]/10 dark:bg-[#418FC8]/20 rounded-bl-full -z-0 opacity-50 group-hover:scale-110 transition-transform duration-500"></div>

        <!-- Cabecera de la Tarjeta -->
        <div class="flex justify-between items-start mb-5 relative z-10">
          <div>
            <div class="flex items-center gap-2 mb-1.5">
              <span class="px-2.5 py-1 bg-[#418FC8]/10 dark:bg-[#418FC8]/20 text-[#418FC8] dark:text-[#6DC7DC] text-xs font-bold rounded-lg flex items-center gap-1.5">
                <PhCalendarBlank class="h-3.5 w-3.5" />
                {{ formatDate(receta.date) }}
              </span>
              <span class="text-xs font-semibold text-slate-400">#{{ receta.id }}</span>
            </div>
            <h3 class="text-lg font-bold text-slate-800 dark:text-white">{{ receta.doctor }}</h3>
            <p class="text-sm font-medium text-[#418FC8] dark:text-[#6DC7DC]">{{ receta.specialty }}</p>
          </div>
          
          <!-- Ícono de Receta -->
          <div class="w-10 h-10 bg-slate-50 dark:bg-slate-700 rounded-full flex items-center justify-center border border-slate-100 dark:border-slate-600 text-slate-400 dark:text-slate-500">
            <PhFileText class="h-5 w-5" />
          </div>
        </div>

        <div class="mb-4 relative z-10">
          <p class="text-sm text-slate-600 dark:text-slate-300"><span class="font-bold text-slate-700 dark:text-slate-200">Diagnóstico:</span> {{ receta.diagnosis }}</p>
        </div>

        <!-- Lista de Medicamentos -->
        <div class="bg-slate-50 dark:bg-slate-700/50 rounded-2xl p-4 mb-5 flex-1 relative z-10 border border-slate-100 dark:border-slate-600">
          ...
          <ul class="space-y-3">
            <li v-for="(med, index) in receta.medications" :key="index" class="flex items-start gap-3">
              ...
              <div>
                <p class="text-sm font-bold text-slate-800 dark:text-white">{{ med.name }} <span class="text-[#418FC8] dark:text-[#6DC7DC] font-semibold text-xs ml-1">{{ med.dosage }}</span></p>
                <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{{ med.frequency }} durante {{ med.duration }}</p>
              </div>
            </li>
          </ul>
        </div>
        ...
        <div v-if="receta.notes" class="mb-5 relative z-10">
          <p class="text-xs text-slate-500 dark:text-slate-400 italic bg-amber-50 dark:bg-amber-900/40 text-amber-800 dark:text-amber-300 p-3 rounded-xl border border-amber-100 dark:border-amber-800">
        ...
        <div class="mt-auto pt-2 relative z-10">
          <button 
            ...
            :class="downloadingId === receta.id 
              ...
              : 'bg-white dark:bg-slate-700 border-2 border-[#418FC8] dark:border-[#6DC7DC] text-[#418FC8] dark:text-[#6DC7DC] hover:bg-[#418FC8] dark:hover:bg-[#418FC8] hover:text-white shadow-sm hover:shadow-md hover:-translate-y-0.5'"
          >
    ...
    <div v-else class="flex flex-col items-center justify-center py-16 px-4 text-center bg-white dark:bg-slate-800 rounded-3xl border border-dashed border-slate-300 dark:border-slate-700">
      <div class="w-20 h-20 bg-slate-50 dark:bg-slate-700 rounded-full flex items-center justify-center mb-4">
        <PhFolderOpen class="h-10 w-10 text-slate-300 dark:text-slate-500" />
      </div>
      <h3 class="text-lg font-bold text-slate-700 dark:text-slate-200 mb-2">Sin recetas médicas</h3>
      <p class="text-slate-500 dark:text-slate-400 max-w-md">No tienes recetas médicas emitidas actualmente. Cuando un especialista te prescriba medicamentos, aparecerán aquí.</p>
    </div>
