...
<template>
  <div class="max-w-7xl mx-auto w-full px-4 sm:px-0 font-sans">
    <div class="mb-8">
      <h1 class="text-3xl font-extrabold text-slate-800 tracking-tight">Diagnósticos Clínicos Emitidos</h1>
      <p class="text-slate-500 mt-2 font-medium">Historial completo de los diagnósticos que has registrado a tus pacientes.</p>
    </div>
    ...
    <div class="flex flex-col sm:flex-row gap-4 mb-6">
      <div class="relative w-full sm:flex-1 sm:max-w-md">
        ...
        <input type="text" placeholder="Buscar por nombre o CIE-10..." class="w-full pl-11 pr-4 py-3 bg-white border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#6DC7DC]/50 focus:border-[#418FC8] font-medium text-sm text-slate-700" />
      </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
      ...
      <div class="block lg:hidden divide-y divide-slate-100">
        <div v-for="diag in diagnoses" :key="'mob-'+diag.id" class="p-5 hover:bg-slate-50 transition-colors group">
          <div class="flex justify-between items-start mb-3">
            <div class="flex items-center gap-3">
              ...
              <div>
                <h3 class="font-bold text-slate-800 text-sm">{{ diag.patient }}</h3>
                <p class="text-xs text-slate-500">{{ diag.date }}</p>
              </div>
            </div>
            <span class="text-xs font-bold text-slate-400">#{{ diag.id.split('-')[1] }}</span>
          </div>
          <div class="mb-4 bg-slate-50 p-3 rounded-xl border border-slate-100">
            <div class="flex items-center gap-2 mb-1">
              <span class="bg-indigo-100 text-indigo-700 font-bold px-2 py-0.5 rounded text-[10px] border border-indigo-200">{{ diag.code }}</span>
            </div>
            <p class="text-sm font-medium text-slate-700">{{ diag.description }}</p>
          </div>
      ...
      <div class="hidden lg:block overflow-x-auto">
        <table class="w-full text-left border-collapse min-w-[800px]">
          ...
          <tbody class="divide-y divide-slate-100">
            <tr v-for="diag in diagnoses" :key="diag.id" class="hover:bg-blue-50/50 transition-colors group">
              <td class="p-5 font-bold text-slate-500 text-sm">{{ diag.id }}</td>
              ...
              <td class="p-5 text-slate-600 font-medium text-sm">{{ diag.date }}</td>
              <td class="p-5">
                <div class="flex items-center gap-2">
                  <span class="bg-indigo-100 text-indigo-700 font-bold px-2 py-0.5 rounded text-xs border border-indigo-200">{{ diag.code }}</span>
                  <span class="text-sm font-medium text-slate-700">{{ diag.description }}</span>
                </div>
              </td>
